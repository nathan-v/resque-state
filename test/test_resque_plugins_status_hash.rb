...
class TestResquePluginsStatusHash < Minitest::Test
  describe 'Resque::Plugins::Status::Hash' do
    before do
      ...
      @uuid_with_json = Resque::Plugins::Status::Hash.create(
        Resque::Plugins::Status::Hash.generate_uuid, 'im' => 'json'
      )
    end
    ...
    describe '.get' do
      ...
      it 'decode encoded json' do
        assert_equal('json',
                     Resque::Plugins::Status::Hash.get(@uuid_with_json)['im'])
      end
    end

    describe '.mget' do
      it 'return statuses array of Resque::Plugins::Status::Hash for uuids' do
        uuid2 = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid
        )
      ...
      it 'decode encoded json' do
        assert_equal ['json'],
                     Resque::Plugins::Status::Hash.mget(
                       [@uuid_with_json]
                     ).map { |h| h['im'] }
      end
    end
    ...
    describe '.set' do
      ...
      it 'return the status' do
        assert Resque::Plugins::Status::Hash.set(
          @uuid, 'updated'
        ).is_a?(Resque::Plugins::Status::Hash)
      end
    end
    ...
    describe '.create' do
      ...
      it 'store any status passed' do
        uuid = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid, 'initial status'
        )
        status = Resque::Plugins::Status::Hash.get(uuid)
        assert status.is_a?(Resque::Plugins::Status::Hash)
      ...
      it 'expire keys if expire_in is set' do
        ...
        uuid = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid, 'new status'
        )
        assert_includes Resque::Plugins::Status::Hash.status_ids, uuid
        assert_equal 'new status',
                     Resque::Plugins::Status::Hash.get(uuid).message
        sleep 2
        Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid
        )
    ...
    describe '.clear_completed' do
      before do
        @completed_status_id = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid, 'status' => 'completed'
        )
        @not_completed_status_id = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid
        )
    ...
    describe '.clear_failed' do
      before do
        @failed_status_id = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid, 'status' => 'failed'
        )
        @not_failed_status_id = Resque::Plugins::Status::Hash.create(
          Resque::Plugins::Status::Hash.generate_uuid
        )
    ...
    describe '.status_ids' do
      ...
      it 'let you paginate through the statuses' do
        assert_equal Resque::Plugins::Status::Hash.status_ids[0, 10],
                     Resque::Plugins::Status::Hash.status_ids(0, 9)
        assert_equal Resque::Plugins::Status::Hash.status_ids[10, 10],
                     Resque::Plugins::Status::Hash.status_ids(10, 19)
        # assert_equal Resque::Plugins::Status::Hash.status_ids.reverse[0, 10],
        #              Resque::Plugins::Status::Hash.status_ids(0, 10)
      end
    end
    ...
    Resque::Plugins::Status::STATUSES.each do |status_code|
      describe ".#{status_code}?" do
        ...
        it 'return false for other statuses' do
          other_codes = Resque::Plugins::Status::STATUSES - [status_code]
